...
const globalCode = generateSessionId(6);
    ...
    <div className="font-lato bg-gradient-to-t from-gray-200 to-white min-h-screen">
      ...
      <div class="mt-20 mb-4 mx-12 flex flex-col md:flex-row items-center justify-between gap-10">
        <div class="flex flex-col max-w-xl">
          ...
          <div className="mt-7">
            <button onClick={() => navigate('/login')} className="px-6 py-2.5 rounded-lg bg-black text-white hover:bg-gray-800">
              Create a Hangout
            </button>
            <button onClick={() => navigate('/login')} className="ml-4 px-9 py-2.5 rounded-lg bg-white border border-gray-200 hover:bg-gray-100">
              Join a Hangout
            </button>
          </div>
        </div>
...
function CreateSession(){
  ...
  const handleSubmit = (e) => {
    ...
    setHangoutCode(globalCode); // Generate a random hangout code

    // Later: You can POST hangoutData to backend here

    // navigate('/hangout/12345'); // temporary dummy ID
    setTimeout(() => navigate('/session/join'), 5000);
  };
    ...
    <div class="font-lato items-center px-96 pt-6 py-3 bg-gradient-to-t from-gray-200 to-white min-h-screen">
      ...
      <div class="mt-2 border-2 border-gray-300 shadow-lg rounded-lg p-6 pb-8 bg-white">
        ...
        <form class="mt-4" id="create_hangout_form" onSubmit={handleSubmit}>
          ...
          <div className="mt-4">
            <label htmlFor="hangout_code">Your Hangout Code:</label>
            <p id="hangout_code" className="font-bold text-lg text-gray-800">{hangoutCode}</p>
          </div>
          </form>
      </div>
    </div>
...
function JoinSession(){
  ...
  return(
    <div class="font-lato items-center px-96 pt-10 bg-gradient-to-t from-gray-200 to-white min-h-screen">
      ...
      <div class="mx-20 mt-2 border-2 border-gray-300 shadow-lg rounded-lg p-6 pb-14 bg-white">
...
function LoggedinHome() {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (response.ok) {
        setMessage2('Logging out...');
        setTimeout(() => navigate('/'), 2000);
        // navigate('/'); // Go back to home after successful logout
      } else {
  ...
  return (
    ...
    <div className="font-lato bg-gradient-to-t from-gray-200 to-white min-h-screen">
      ...
      <nav className="flex justify-between items-center px-8 py-3 bg-white shadow-sm">
        ...
        <div>
          {/* <button className="px-4 py-2" onClick={() => navigate('/login')}>Login</button> */}
          <button className="px-4 py-2 rounded-lg bg-black text-white hover:bg-gray-800" onClick={handleLogout}>Logout</button>
          <h1>{message2}</h1>
        </div>
      </nav>
...
function ItineraryTab() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    alert('Hangout successfully created!');
    navigate('/dashboard');
  };
  return (
    <div>
    <form onSubmit={handleSubmit} method="post" class="max-w-md mx-auto bg-white shadow-lg rounded-xl p-8 space-y-6">
      <h2 class="text-2xl font-bold text-center text-gray-800">Create an Activity</h2>

      <div>
        <label for="location" class="block text-sm font-medium text-gray-700">Location:<font color="red">*</font></label>
        <input type="text" id="location" name="location" required
          class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 px-3 py-2"/>
      </div>

      <div>
        <label for="activity" class="block text-sm font-medium text-gray-700">Activity Description:</label>
        <textarea id="activity" name="activity" rows="4"
          class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 px-3 py-2"></textarea>
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700">Time:<font color="red">*</font></label>
        <div class="flex space-x-2 mt-1">
          <select id="hour" name="hour" required
            class="block w-1/3 rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 px-2 py-2">
          ...
          <select id="minute" name="minute" required
            ...
            <option>25</option><option>30</option><option>35</option><option>40</option><option>45</option><option>50</option><option>55</option>
          </select>
      ...
      <div>
        <button type="submit"
          class="w-full bg-black text-white font-semibold py-2 rounded-lg shadow-md hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-opacity-75">
          Submit
        </button>
      </div>
    </form>
...
function App(){
  return(
    <Routes>
      ...
      <Route path="/session/:id" element={<HangoutPage />} />
